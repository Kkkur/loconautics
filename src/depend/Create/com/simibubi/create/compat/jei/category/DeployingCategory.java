...
extends CreateRecipeCategory<DeployerApplicationRecipe> {
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, DeployerApplicationRecipe recipe, IFocusGroup focuses) {
        ...
        for (int i = 0; i < results.size(); ++i) {
            ...
            int yOffset = i / 2 * -19;
            ((IRecipeSlotBuilder)builder.addSlot(RecipeIngredientRole.OUTPUT, single ? 132 : 132 + xOffset, 51 + yOffset).setBackground(DeployingCategory.getRenderedSlot(output), -1, -1).addItemStack(output.getStack())).addRichTooltipCallback(DeployingCategory.addStochasticTooltip(output));
        }
        if (recipe.shouldKeepHeldItem()) {
            handItemSlot.addTooltipCallback((recipeSlotView, tooltip) -> tooltip.add(1, CreateLang.translateDirect("recipe.deploying.not_consumed", new Object[0]).withStyle(ChatFormatting.GOLD)));
        }
    }
