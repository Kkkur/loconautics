...
extends DisplaySource {
    ...
    public List<MutableComponent> provideText(DisplayLinkContext context, DisplayTargetStats stats) {
        ...
        List<MutableComponent> list = this.provideEntries(context, stats.maxRows() * (isBook ? 8 : 1)).map(e -> this.createComponentsFromEntry(context, (IntAttached<MutableComponent>)e)).map(l -> {
    ...
    private List<MutableComponent> condensePages(List<MutableComponent> list) {
        ArrayList<MutableComponent> condensed = new ArrayList<MutableComponent>();
        ...
        for (int i = 0; i < list.size(); ++i) {
            ...
            current.append((Component)Component.literal((String)"\n")).append((Component)atIndex);
            if ((i + 1) % 8 != 0) continue;
    ...
    protected List<MutableComponent> createComponentsFromEntry(DisplayLinkContext context, IntAttached<MutableComponent> entry) {
        ...
        if (this.shortenNumbers(context)) {
            ...
            return this.valueFirst() ? Arrays.asList((MutableComponent)shortened.getFirst(), (MutableComponent)shortened.getSecond(), name) : Arrays.asList(name, (MutableComponent)shortened.getFirst(), (MutableComponent)shortened.getSecond());
        }
        MutableComponent formattedNumber = Component.literal((String)String.valueOf(number)).append((Component)WHITESPACE);
    ...
    private Couple<MutableComponent> shorten(int number) {
        if (number >= 1000000) {
            return Couple.create((Object)Component.literal((String)String.valueOf(number / 1000000)), (Object)CreateLang.translateDirect("display_source.value_list.million", new Object[0]).append((Component)WHITESPACE));
        }
        if (number >= 1000) {
            return Couple.create((Object)Component.literal((String)String.valueOf(number / 1000)), (Object)CreateLang.translateDirect("display_source.value_list.thousand", new Object[0]).append((Component)WHITESPACE));
        }
        return Couple.create((Object)Component.literal((String)String.valueOf(number)), (Object)WHITESPACE);
    }
    ...
    protected void addFullNumberConfig(ModularGuiLineBuilder builder) {
        builder.addSelectionScrollInput(0, 75, (si, l) -> si.forOptions(CreateLang.translatedOptions("display_source.value_list", "shortened", "full_number")).titled(CreateLang.translateDirect("display_source.value_list.display", new Object[0])), "Format");
    }
}
