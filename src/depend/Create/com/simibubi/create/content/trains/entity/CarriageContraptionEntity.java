...
extends OrientedContraptionEntity {
    private static final EntityDataAccessor<CarriageSyncData> CARRIAGE_DATA = SynchedEntityData.defineId(CarriageContraptionEntity.class, (EntityDataSerializer)AllEntityDataSerializers.CARRIAGE_DATA);
    private static final EntityDataAccessor<Optional<UUID>> TRACK_GRAPH = SynchedEntityData.defineId(CarriageContraptionEntity.class, (EntityDataSerializer)EntityDataSerializers.OPTIONAL_UUID);
    ...
    public CarriageContraptionEntity(EntityType<?> type, Level world) {
        ...
        this.derailParticleOffset = VecHelper.offsetRandomly((Vec3)Vec3.ZERO, (RandomSource)world.random, (float)1.5f).multiply(1.0, 0.25, 1.0);
    }
    ...
    public void setBlock(BlockPos localPos, StructureTemplate.StructureBlockInfo newInfo) {
        ...
        this.carriage.forEachPresentEntity(cce -> {
            ...
            CatnipServices.NETWORK.sendToClientsTrackingEntity((Entity)cce, (CustomPacketPayload)new ContraptionBlockChangedPacket(cce.getId(), localPos, newInfo.state()));
        });
    }
    ...
    protected void tickContraption() {
        ...
        if (!this.level().isClientSide) {
            ...
            if (shouldCarriageSyncThisTick && carriageData.isDirty()) {
                ...
                this.entityData.set(CARRIAGE_DATA, (Object)carriageData);
        ...
        if (!this.firstPositionUpdate) {
            ...
            Vec3 relativeDiff = VecHelper.rotate((Vec3)diff, (double)this.yaw, (Direction.Axis)Direction.Axis.Y);
    ...
    protected void addPassenger(Entity pPassenger) {
        ...
        player.getPersistentData().put("ContraptionMountLocation", (Tag)VecHelper.writeNBT((Vec3)player.position()));
    }
    ...
    private void spawnPortalParticles(Carriage.DimensionalCarriageEntity dce) {
        ...
        for (BlockPos pos : this.particleSlice) {
            ...
            m = VecHelper.rotate((Vec3)m, (double)(this.random.nextFloat() * 360.0f), (Direction.Axis)(alongX ? Direction.Axis.X : Direction.Axis.Z));
            m = m.add(VecHelper.offsetRandomly((Vec3)Vec3.ZERO, (RandomSource)this.random, (float)0.25f));
    ...
    public Couple<Boolean> checkConductors() {
        Couple sides = Couple.create((Object)false, (Object)false);
        Contraption contraption = this.contraption;
        if (!(contraption instanceof CarriageContraption)) {
            return sides;
        }
        CarriageContraption cc = (CarriageContraption)contraption;
        sides.setFirst((Object)((Boolean)cc.blockConductors.getFirst()));
        sides.setSecond((Object)((Boolean)cc.blockConductors.getSecond()));
        for (Entity entity : this.getPassengers()) {
            ...
            sides.setFirst((Object)((Boolean)sides.getFirst() != false || (Boolean)validSides.getFirst() != false ? 1 : 0));
            sides.setSecond((Object)((Boolean)sides.getSecond() != false || (Boolean)validSides.getSecond() != false ? 1 : 0));
        }
        return sides;
    }
    ...
    public Component getDisplayName() {
        if (this.carriage == null) {
            return CreateLang.translateDirect("train", new Object[0]);
        }
        return this.carriage.train.name;
    }
    ...
    public boolean control(BlockPos controlsLocalPos, Collection<Integer> heldControls, Player player) {
        ...
        if (!this.canInteractWithBlock(player, VecHelper.getCenterOf((Vec3i)controlsLocalPos), 8.0)) {
            return false;
        }
        ...
        if (this.hudPacketCooldown-- <= 0 && player instanceof ServerPlayer) {
            ...
            CatnipServices.NETWORK.sendToClient(sp, (CustomPacketPayload)new TrainHUDUpdatePacket.Clientbound(this.carriage.train));
        ...
        if (currentStation != null && spaceDown) {
            this.sendPrompt(player, CreateLang.translateDirect("train.arrived_at", Component.literal((String)currentStation.name).withStyle(s -> s.withColor(7358000))), false);
            return true;
        }
        ...
        if (currentStation != null && targetSpeed != 0) {
            ...
            this.sendPrompt(player, CreateLang.translateDirect("train.departing_from", Component.literal((String)currentStation.name).withStyle(s -> s.withColor(7358000))), false);
        }
        if (currentStation == null) {
            ...
            if (nav.destination != null) {
                ...
                if (spaceDown) {
                    ...
                    int toColor = 5413141;
                    int mixedColor = Color.mixColors((int)toColor, (int)fromColor, (float)((float)progress / 30.0f));
                    int targetColor = arrived ? toColor : 0x544D45;
                    MutableComponent component = greenComponent.withStyle(st -> st.withColor(mixedColor)).append((Component)whiteComponent.withStyle(st -> st.withColor(targetColor)));
    ...
    private void sendPrompt(Player player, MutableComponent component, boolean shadow) {
        if (player instanceof ServerPlayer) {
            ServerPlayer sp = (ServerPlayer)player;
            CatnipServices.NETWORK.sendToClient(sp, (CustomPacketPayload)new TrainPromptPacket((Component)component, shadow));
        }
    }

    private void displayApproachStationMessage(Player player, GlobalStation station) {
        this.sendPrompt(player, CreateLang.translateDirect("contraption.controls.approach_station", Component.keybind((String)"key.jump"), station.name), false);
    ...
    private void updateTrackGraph() {
        ...
        TrackGraph graph = CreateClient.RAILWAYS.sided((LevelAccessor)this.level()).trackNetworks.get(optional.get());
        if (graph == null) {
            return;
        }
