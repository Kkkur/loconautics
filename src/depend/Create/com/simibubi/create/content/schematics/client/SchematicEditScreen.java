...
extends AbstractSimiScreen {
    ...
    protected void init() {
        ...
        this.addRenderableWidgets((GuiEventListener[])new AbstractSimiWidget[]{labelR, labelM, this.rotationArea, this.mirrorArea});
    ...
    public boolean keyPressed(int code, int p_keyPressed_2_, int p_keyPressed_3_) {
        String coords;
        if (SchematicEditScreen.isPaste((int)code) && (coords = this.minecraft.keyboardHandler.getClipboard()) != null && !coords.isEmpty()) {
            coords.replaceAll(" ", "");
    ...
    protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        graphics.drawString(this.font, title, x + (this.background.getWidth() - 8 - this.font.width(title)) / 2, y + 4, 0x505050, false);
        ((GuiGameElement.GuiRenderBuilder)GuiGameElement.of((ItemStack)AllItems.SCHEMATIC.asStack()).at((float)(x + this.background.getWidth() + 6), (float)(y + this.background.getHeight() - 40), -200.0f)).scale(3.0).render(graphics);
    }
