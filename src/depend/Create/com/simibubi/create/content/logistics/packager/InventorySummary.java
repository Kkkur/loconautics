...
public class InventorySummary {
    ...
    public void add(ItemStack stack, int count) {
        ...
        List stacks = this.items.computeIfAbsent(stack.getItem(), $ -> Lists.newArrayList());
        for (BigItemStack existing : stacks) {
            ItemStack existingStack = existing.stack;
            if (!ItemStack.isSameItemSameComponents((ItemStack)existingStack, (ItemStack)stack)) continue;
    ...
    public int getCountOf(ItemStack stack) {
        ...
        for (BigItemStack entry : list) {
            if (!ItemStack.isSameItemSameComponents((ItemStack)entry.stack, (ItemStack)stack)) continue;
            return entry.count;
        }
        return 0;
    }
    ...
    public void divideAndSendTo(ServerPlayer player, BlockPos pos) {
        ...
        if (stacks.isEmpty()) {
            CatnipServices.NETWORK.sendToClient(player, (CustomPacketPayload)new LogisticalStockResponsePacket(true, pos, Collections.emptyList()));
        }
        for (BigItemStack entry : stacks) {
            ...
            CatnipServices.NETWORK.sendToClient(player, (CustomPacketPayload)new LogisticalStockResponsePacket(false, pos, currentList));
            currentList = null;
        }
        if (currentList != null) {
            CatnipServices.NETWORK.sendToClient(player, (CustomPacketPayload)new LogisticalStockResponsePacket(true, pos, (List<BigItemStack>)currentList));
        }
    }
