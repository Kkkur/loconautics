...
public class TrainScenes {
    public static void controls(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay().chaseBoundingBoxOutline(PonderPalette.WHITE, (Object)train, new AABB(initialControlsPos).contract(-0.375, 0.125, 0.0), 85);
        scene.idle(15);
    ...
    public static void schedule(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.setSceneOffsetY(-1.0f);
        scene.showBasePlate();
        scene.world().cycleBlockProperty(util.grid().at(3, 3, 4), (Property)BlazeBurnerBlock.HEAT_LEVEL);
        for (int i = 10; i >= 0; --i) {
            scene.world().showSection(util.select().position(i, 1, 4), Direction.DOWN);
            scene.idle(1);
        }
        ...
        scene.idle(80);
        scene.overlay().showControls(util.vector().centerOf(util.grid().at(3, 1, 7)), Pointing.DOWN, 30).withItem(new ItemStack((ItemLike)Items.LEAD));
        scene.idle(40);
        target = util.vector().centerOf(util.grid().at(3, 3, 4));
        scene.overlay().showControls(target.add(0.5, 0.0, 0.0), Pointing.RIGHT, 30).rightClick().withItem(new ItemStack((ItemLike)Items.LEAD));
        scene.idle(6);
