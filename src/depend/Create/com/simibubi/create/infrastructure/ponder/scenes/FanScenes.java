...
public class FanScenes {
    ...
    public static void processing(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.idle(10);
        scene.overlay().showOutlineWithText(util.select().fromTo(blockPos, blockPos.west(2)), 80).colored(PonderPalette.RED).text("When passing through lava, the Air Flow becomes Heated");
        scene.idle(80);
        ItemStack stack = new ItemStack((ItemLike)Items.GOLD_ORE);
        ItemStack smelted = new ItemStack((ItemLike)Items.GOLD_INGOT);
        ElementLink entityLink = scene.world().createItemEntity(util.vector().centerOf(blockPos.west(2).above(2)), util.vector().of(0.0, 0.1, 0.0), stack);
        scene.idle(15);
        scene.world().modifyEntity(entityLink, e -> e.setDeltaMovement((double)-0.2f, 0.0, 0.0));
        ...
        scene.idle(20);
        scene.effects().emitParticles(itemVec.add(0.0, (double)0.2f, 0.0), scene.effects().simpleParticleEmitter((ParticleOptions)ParticleTypes.LARGE_SMOKE, Vec3.ZERO), 1.0f, 60);
        ...
        scene.world().moveSection(blockInFront, util.vector().of(1.0, 0.0, 2.0), 0);
        scene.idle(50);
        scene.overlay().showOutlineWithText(util.select().fromTo(blockPos, blockPos.west(2)), 60).colored(PonderPalette.BLACK).attachKeyFrame().text("Instead, a setup for Smoking using Fire should be used for them");
        scene.idle(80);
        ...
        scene.world().moveSection(blockInFront, util.vector().of(1.0, 0.0, 2.0), 0);
        scene.idle(20);
        scene.overlay().showOutlineWithText(util.select().fromTo(blockPos, blockPos.west(2)), 60).colored(PonderPalette.MEDIUM).attachKeyFrame().text("Air Flows passing through water create a Washing Setup");
        scene.idle(70);
        stack = new ItemStack((ItemLike)Items.RED_SAND, 16);
        ItemStack washed = new ItemStack((ItemLike)Items.GOLD_NUGGET, 16);
        entityLink = scene.world().createItemEntity(util.vector().centerOf(blockPos.west(2).above(2)), util.vector().of(0.0, 0.1, 0.0), stack);
        scene.idle(15);
        scene.world().modifyEntity(entityLink, e -> e.setDeltaMovement((double)-0.2f, 0.0, 0.0));
        scene.overlay().showControls(itemVec, Pointing.DOWN, 20).withItem(stack);
        scene.idle(20);
        scene.effects().emitParticles(itemVec.add(0.0, (double)0.2f, 0.0), scene.effects().simpleParticleEmitter((ParticleOptions)ParticleTypes.SPIT, Vec3.ZERO), 1.0f, 60);
        ...
        ItemStack sand = new ItemStack((ItemLike)Items.SAND);
        ItemStack clay = new ItemStack((ItemLike)Items.CLAY_BALL);
        scene.idle(20);
        ...
        scene.effects().emitParticles(depotTop, scene.effects().simpleParticleEmitter((ParticleOptions)ParticleTypes.SPIT, Vec3.ZERO), 0.5f, 30);
        scene.idle(30);
        scene.world().modifyBlockEntityNBT(util.select().position(depos), DepotBlockEntity.class, nbt -> nbt.put("HeldItem", (Tag)new TransportedItemStack(clay).serializeNBT(scene.world().getHolderLookupProvider())));
        scene.effects().emitParticles(depotTop, scene.effects().simpleParticleEmitter((ParticleOptions)ParticleTypes.SPIT, Vec3.ZERO), 0.5f, 30);
        ...
        scene.idle(60);
        scene.effects().emitParticles(depotTop, scene.effects().simpleParticleEmitter((ParticleOptions)ParticleTypes.SPIT, Vec3.ZERO), 0.5f, 25);
        scene.idle(25);
        scene.world().changeBeltItemTo(transported, new ItemStack((ItemLike)Items.CLAY_BALL));
        scene.effects().emitParticles(depotTop, scene.effects().simpleParticleEmitter((ParticleOptions)ParticleTypes.SPIT, Vec3.ZERO), 0.5f, 25);
        scene.idle(60);
