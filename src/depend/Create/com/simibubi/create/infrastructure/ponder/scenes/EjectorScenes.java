...
public class EjectorScenes {
    public static void ejector(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world().modifyBlockEntityNBT(ejectorS, EjectorBlockEntity.class, nbt -> {
            NBTHelper.writeEnum((CompoundTag)nbt, (String)"State", (Enum)EjectorBlockEntity.State.RETRACTING);
        ...
        scene.idle(70);
        scene.overlay().showOutlineWithText(util.select().position(ejectorPos.west()), 70).colored(PonderPalette.OUTPUT).text("If no valid Target was selected, it will simply target the block directly in front").placeNearTarget();
        scene.idle(80);
        ...
        scene.overlay().showControls(util.vector().topOf(ejectorPos).add(0.5, 0.0, 0.0), Pointing.RIGHT, 30).withItem(copperBlock);
        scene.idle(7);
        ...
        scene.world().modifyEntities(ItemEntity.class, Entity::discard);
        scene.addKeyframe();
        ElementLink<ParrotElement> birb = scene.special().createBirb(util.vector().topOf(ejectorPos).add(0.0, -0.1875, 0.0), ParrotPose.FlappyPose::new);
        scene.idle(15);
    ...
    public static void splitY(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay().showControls(util.vector().topOf(tunnel), Pointing.DOWN, 80).showing((ScreenElement)AllIcons.I_TUNNEL_PREFER_NEAREST);
        scene.idle(10);
        ...
        scene.idle(90);
        scene.overlay().showControls(util.vector().topOf(util.grid().at(4, 1, 3)), Pointing.DOWN, 20).withItem(new ItemStack((ItemLike)Items.COPPER_INGOT));
        scene.world().showSection(coverbelt, Direction.SOUTH);
        scene.idle(7);
        scene.world().createItemOnBelt(util.grid().at(4, 1, 3), Direction.UP, new ItemStack((ItemLike)Items.COPPER_INGOT, 64));
        scene.idle(40);
    ...
    public static void redstone(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.idle(30);
        ElementLink observer = scene.world().showIndependentSection(util.select().position(4, 1, 1), Direction.SOUTH);
