...
public class DrainScenes {
    public static void emptying(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        ElementLink drainLink = scene.world().showIndependentSection(drain, Direction.DOWN);
        ...
        scene.world().modifyBlockEntity(drainPos, ItemDrainBlockEntity.class, be -> {
            ...
            IFluidHandler fh = (IFluidHandler)be.getLevel().getCapability(Capabilities.FluidHandler.BLOCK, be.getBlockPos(), null);
            if (fh != null) {
                fh.fill(new FluidStack((Fluid)Fluids.LAVA, 1000), IFluidHandler.FluidAction.EXECUTE);
            }
        });
        scene.idle(10);
        ...
        scene.world().modifyBlockEntity(drainPos, ItemDrainBlockEntity.class, be -> {
            IFluidHandler fh = (IFluidHandler)be.getLevel().getCapability(Capabilities.FluidHandler.BLOCK, be.getBlockPos(), null);
            if (fh != null) {
                fh.drain(500, IFluidHandler.FluidAction.EXECUTE);
            }
        });
