...
public class DisplayScenes {
    public static void link(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay().showControls(target, Pointing.RIGHT, 60).withItem(AllBlocks.DISPLAY_LINK.asStack()).rightClick();
        scene.idle(6);
        scene.overlay().chaseBoundingBoxOutline(PonderPalette.OUTPUT, (Object)link, new AABB(board).expandTowards(-2.0, -1.0, 0.0).deflate(0.0, 0.0, 0.1875), 60);
        scene.idle(35);
        ...
        scene.idle(20);
        scene.overlay().showOutlineWithText(depot, 70).text("...then attach it to the block to read from").pointAt(util.vector().centerOf(linkPos)).colored(PonderPalette.INPUT).placeNearTarget();
        scene.idle(60);
        ...
        scene.world().flashDisplayLink(linkPos);
        scene.idle(20);
        scene.overlay().showText(80).text("The display will now receive information from the link").pointAt(target.add((double)-2.45f, -0.5, 0.0)).attachKeyFrame().placeNearTarget();
        scene.idle(30);
        scene.world().removeItemsFromBelt(depotPos);
        ...
        scene.world().flashDisplayLink(linkPos);
        scene.idle(5);
        ElementLink dirtElement = scene.world().showIndependentSection(dirt, Direction.SOUTH);
        scene.world().moveSection(dirtElement, util.vector().of(0.0, -4.0, 0.0), 0);
        scene.idle(25);
        scene.overlay().showOutlineWithText(depot, 50).text("Not every block can act as a source").pointAt(util.vector().topOf(depotPos)).attachKeyFrame().colored(PonderPalette.RED).placeNearTarget();
        scene.idle(60);
        scene.world().hideIndependentSection(dirtElement, Direction.SOUTH);
        scene.idle(10);
        ElementLink stressElement = scene.world().showIndependentSection(stresso, Direction.SOUTH);
        scene.world().moveSection(stressElement, util.vector().of(0.0, -2.0, 0.0), 0);
        scene.idle(10);
        scene.world().setDisplayBoardText(board, 1, (Component)Component.literal((String)"1024 ").append((Component)CreateLang.translateDirect("generic.unit.stress", new Object[0])));
        scene.world().flashDisplayLink(linkPos);
        scene.idle(40);
        scene.world().hideIndependentSection(stressElement, Direction.SOUTH);
        scene.idle(10);
        ElementLink chestElement = scene.world().showIndependentSection(content, Direction.SOUTH);
        scene.world().moveSection(chestElement, util.vector().of(0.0, -3.0, 0.0), 0);
        scene.idle(10);
        scene.world().setDisplayBoardText(board, 1, (Component)Component.literal((String)"418 ").append(new ItemStack((ItemLike)Items.DEEPSLATE).getHoverName()));
        scene.world().setDisplayBoardText(board, 2, (Component)Component.literal((String)"14 ").append(AllBlocks.COGWHEEL.asStack().getHoverName()));
        scene.world().flashDisplayLink(linkPos);
        scene.idle(40);
        ...
        ElementLink cuckooElement = scene.world().showIndependentSection(cuckoo, Direction.SOUTH);
        scene.world().moveSection(cuckooElement, util.vector().of(0.0, -1.0, 0.0), 0);
        scene.idle(10);
        scene.world().setDisplayBoardText(board, 1, (Component)Component.literal((String)"6:00 ").append((Component)CreateLang.translateDirect("generic.daytime.pm", new Object[0])));
        ...
        scene.idle(10);
        ElementLink signElement = scene.world().showIndependentSection(sign, Direction.SOUTH);
        ...
        scene.idle(10);
        ElementLink lecternElement = scene.world().showIndependentSection(lectern, Direction.SOUTH);
        ...
        ElementLink nixieElement = scene.world().showIndependentSection(nixies, Direction.SOUTH);
    ...
    public static void board(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.idle(6);
        scene.world().setDisplayBoardText(board, 0, (Component)Component.literal((String)"Create"));
        scene.idle(25);
        ...
        scene.world().showSection(link, Direction.EAST);
        scene.idle(15);
        scene.overlay().chaseBoundingBoxOutline(PonderPalette.INPUT, (Object)depot, new AABB(linkPos).contract(-0.5, 0.0, 0.0), 60);
        scene.idle(5);
        scene.overlay().chaseBoundingBoxOutline(PonderPalette.OUTPUT, (Object)link, new AABB(board).expandTowards(-2.0, -1.0, 0.0).deflate(0.0, 0.0, 0.1875), 60);
        scene.idle(20);
        ...
        scene.world().flashDisplayLink(linkPos);
        scene.idle(50);
        scene.overlay().showControls(target, Pointing.RIGHT, 40).withItem(new ItemStack((ItemLike)Items.PINK_DYE)).rightClick();
        scene.idle(6);
        ...
        scene.idle(25);
        scene.overlay().showControls(target.add(0.0, -0.5, 0.0), Pointing.RIGHT, 40).withItem(new ItemStack((ItemLike)Items.LIME_DYE)).rightClick();
        scene.idle(6);
    ...
    public static void redstone(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.idle(10);
        scene.world().flashDisplayLink(linkPos);
        scene.world().setDisplayBoardText(board, 1, new ItemStack((ItemLike)Items.CALCITE).getHoverName());
        scene.idle(10);
        ...
        scene.idle(50);
        scene.world().removeItemsFromBelt(depotPos);
        ItemStack item = new ItemStack((ItemLike)Items.AMETHYST_BLOCK);
        scene.world().createItemOnBeltLike(depotPos, Direction.SOUTH, item);
        scene.idle(20);
        scene.idle(30);
        scene.world().removeItemsFromBelt(depotPos);
        item = new ItemStack((ItemLike)Items.GOLD_BLOCK);
        scene.world().createItemOnBeltLike(depotPos, Direction.SOUTH, item);
        scene.idle(50);
        scene.world().toggleRedstonePower(redstone);
        scene.world().toggleRedstonePower(link);
        ...
        scene.idle(10);
        ElementLink redstoneBlock = scene.world().showIndependentSection(util.select().position(4, 1, 0), Direction.SOUTH);
        scene.world().moveSection(redstoneBlock, util.vector().of(-1.0, 0.0, 1.0), 0);
        scene.idle(10);
        scene.world().flashDisplayLink(linkPos);
        scene.world().setDisplayBoardText(board, 1, CommonComponents.EMPTY);
        scene.idle(25);
        scene.overlay().showOutlineWithText(depot, 80).colored(PonderPalette.RED).attachKeyFrame().pointAt(util.vector().topOf(linkPos)).placeNearTarget().text("Signals emitted from the source do not affect the Link");
        scene.idle(70);
    }
}
