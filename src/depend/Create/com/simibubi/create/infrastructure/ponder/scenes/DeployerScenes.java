...
public class DeployerScenes {
    public static void filter(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world().moveDeployer(deployerPos, -1.0f, 25);
        scene.idle(44);
        scene.overlay().showOutlineWithText(util.select().position(deployerPos.west(2)), 60).text("It will always interact with the position 2 blocks in front of itself").attachKeyFrame().placeNearTarget().colored(PonderPalette.GREEN).attachKeyFrame();
        scene.world().moveDeployer(deployerPos, 1.0f, 25);
        scene.idle(26);
        scene.world().moveDeployer(deployerPos, -1.0f, 25);
        scene.idle(20);
        ...
        scene.idle(50);
        ItemStack pot = new ItemStack((ItemLike)Items.FLOWER_POT);
        ...
        scene.world().modifyBlockEntityNBT(deployerSelection, teType, nbt -> nbt.put("HeldItem", ItemStack.EMPTY.saveOptional(scene.world().getHolderLookupProvider())));
        scene.world().moveDeployer(deployerPos, -1.0f, 25);
        scene.idle(20);
        scene.world().showSection(util.select().position(deployerPos.above()), Direction.DOWN);
        ItemStack tulip = new ItemStack((ItemLike)Items.RED_TULIP);
        Vec3 entitySpawn = util.vector().topOf(deployerPos.above(3));
        ElementLink entity1 = scene.world().createItemEntity(entitySpawn, util.vector().of(0.0, 0.2, 0.0), tulip);
        scene.idle(17);
        ...
        ElementLink sheep = scene.world().createEntity(w -> {
            Sheep entity = (Sheep)EntityType.SHEEP.create(w);
            ...
            entity.yHeadRotO = 210.0f;
            entity.yHeadRot = 210.0f;
            return entity;
        });
        scene.idle(20);
        ...
        scene.effects().emitParticles(util.vector().topOf(deployerPos.west(2)).add(0.0, -0.25, 0.0), scene.effects().particleEmitterWithinBlockSpace((ParticleOptions)new BlockParticleOption(ParticleTypes.BLOCK, Blocks.PINK_WOOL.defaultBlockState()), util.vector().of(0.0, 0.0, 0.0)), 25.0f, 1);
        scene.world().moveDeployer(deployerPos, -1.0f, 25);
        ...
        scene.world().createItemEntity(util.vector().centerOf(deployerPos.north()).subtract(0.0, 0.45, 0.0), util.vector().of(0.0, 0.0, -0.1), new ItemStack((ItemLike)Items.PINK_WOOL));
    ...
    public static void modes(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.idle(10);
        ItemStack tool = new ItemStack((ItemLike)Items.GOLDEN_HOE);
        scene.overlay().showControls(util.vector().topOf(deployerPos), Pointing.DOWN, 30).withItem(tool);
        scene.idle(7);
        scene.world().modifyBlockEntityNBT(deployerSelection, DeployerBlockEntity.class, nbt -> nbt.put("HeldItem", tool.saveOptional(scene.world().getHolderLookupProvider())));
        scene.idle(45);
        ...
        scene.world().moveDeployer(deployerPos, -1.0f, 25);
        scene.idle(46);
        scene.overlay().showControls(frontVec, Pointing.LEFT, 40).rightClick().withItem(AllItems.WRENCH.asStack());
        scene.idle(7);
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            scene.world().moveDeployer(deployerPos, -1.0f, 25);
            if (i == 3) {
                scene.world().createItemEntity(util.vector().centerOf(breakingPos), util.vector().of(0.0, 0.0, 0.0), new ItemStack((ItemLike)Blocks.DIRT));
            }
            scene.idle(26);
            if (i != 0) continue;
            scene.markAsFinished();
    ...
    public static void processing(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.overlay().showControls(util.vector().blockSurface(pressPos.below(), Direction.EAST).add(0.0, 0.15, 0.0), Pointing.RIGHT, 30).withItem(tool);
        scene.idle(7);
        scene.world().modifyBlockEntityNBT(pressS, DeployerBlockEntity.class, nbt -> nbt.put("HeldItem", tool.saveOptional(scene.world().getHolderLookupProvider())));
        scene.idle(25);
        ...
        scene.idle(30);
        scene.world().moveDeployer(pressPos, -1.0f, 30);
        scene.debug().enqueueCallback(s -> SandPaperItem.spawnParticles(targetV, quartz, (Level)s.getWorld()));
    ...
    public static void contraption(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.idle(5);
        ElementLink pistonHead = scene.world().showIndependentSection(util.select().fromTo(5, 1, 2, 8, 1, 2), Direction.DOWN);
        scene.world().moveSection(pistonHead, util.vector().of(0.0, 0.0, 1.0), 0);
        scene.world().showSection(kinetics, Direction.DOWN);
        scene.idle(5);
        ElementLink contraption = scene.world().showIndependentSection(deployerSelection, Direction.DOWN);
        scene.idle(5);
        ...
        scene.overlay().showOutlineWithText(flowers, 90).attachKeyFrame().colored(PonderPalette.GREEN).text("They activate at each visited location, using items from inventories anywhere on the contraption");
        scene.idle(100);
        ...
        scene.idle(70);
        ItemStack poppy = new ItemStack((ItemLike)Items.POPPY);
