...
public class DebugInformation {
    ...
    private static final ImmutableMap<String, String> mcSystemInfo = (ImmutableMap)Util.make(() -> {
    ...
    public static Collection<InfoElement> listAllOtherMods() {
        ...
        ModList.get().forEachModContainer((id, mod) -> {
            if (!(id.equals("create") || id.equals("neoforge") || id.equals("minecraft") || id.equals("flywheel") || id.equals("ponder"))) {
    ...
    static {
        DebugInfoSection.builder("Create").put("Mod Version", CreateBuildInfo.VERSION).put("Mod Git Commit", CreateBuildInfo.GIT_COMMIT).put("Ponder Version", DebugInformation.getVersionOfMod("ponder")).put("NeoForge Version", DebugInformation.getVersionOfMod("neoforge")).put("Minecraft Version", SharedConstants.getCurrentVersion().getName()).buildTo(DebugInformation::registerBothInfo);
        CatnipServices.PLATFORM.executeOnClientOnly(() -> () -> DebugInfoSection.builder("Graphics").put("Flywheel Version", ModList.get().getModContainerById("flywheel").map(c -> c.getModInfo().getVersion().toString()).orElse("None")).put("Flywheel Backend", () -> Backend.REGISTRY.getIdOrThrow((Object)BackendManager.currentBackend()).toString()).put("OpenGL Renderer", GlUtil::getRenderer).put("OpenGL Version", GlUtil::getOpenGLVersion).put("Graphics Mode", () -> ((GraphicsStatus)Minecraft.getInstance().options.graphicsMode().get()).name().toLowerCase(Locale.ROOT)).put("PojavLauncher Detected", () -> String.valueOf(PojavChecker.IS_PRESENT)).buildTo(DebugInformation::registerClientInfo));
