...
public final class BakedQuadHelper {
    ...
    public static void setXYZ(int[] vertexData, int vertex, Vec3 xyz) {
        vertexData[vertex * BakedQuadHelper.VERTEX_STRIDE + 0] = Float.floatToRawIntBits((float)xyz.x);
        vertexData[vertex * BakedQuadHelper.VERTEX_STRIDE + 1] = Float.floatToRawIntBits((float)xyz.y);
        vertexData[vertex * BakedQuadHelper.VERTEX_STRIDE + 2] = Float.floatToRawIntBits((float)xyz.z);
    }
    ...
    public static void setNormalXYZ(int[] vertexData, int vertex, Vec3 xyz) {
        int data;
        int x = Byte.toUnsignedInt((byte)(Mth.clamp((double)xyz.x, (double)-1.0, (double)1.0) * 127.0));
        int y = Byte.toUnsignedInt((byte)(Mth.clamp((double)xyz.y, (double)-1.0, (double)1.0) * 127.0));
        int z = Byte.toUnsignedInt((byte)(Mth.clamp((double)xyz.z, (double)-1.0, (double)1.0) * 127.0));
