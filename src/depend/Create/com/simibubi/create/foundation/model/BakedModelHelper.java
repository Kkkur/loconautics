...
public class BakedModelHelper {
    public static int[] cropAndMove(int[] vertexData, TextureAtlasSprite sprite, AABB crop, Vec3 move) {
        ...
        for (int vertex = 0; vertex < 4; ++vertex) {
            ...
            Vec3 newXyz = VecHelper.componentMin((Vec3)max, (Vec3)VecHelper.componentMax((Vec3)xyz, (Vec3)min));
            ...
            if (diff.lengthSqr() > 0.0) {
                ...
                BakedQuadHelper.setU(vertexData, vertex, sprite.getU(SpriteShiftEntry.getUnInterpolatedU((TextureAtlasSprite)sprite, (float)u) + uDiff));
                BakedQuadHelper.setV(vertexData, vertex, sprite.getV(SpriteShiftEntry.getUnInterpolatedV((TextureAtlasSprite)sprite, (float)v) + vDiff));
            }
    ...
    public static BakedModel generateModel(BakedModel template, UnaryOperator<TextureAtlasSprite> spriteSwapper) {
        ...
        for (Direction cullFace : Iterate.directions) {
            random.setSeed(42L);
            List quads = template.getQuads(null, cullFace, random, ModelData.EMPTY, RenderType.solid());
            culledFaces.put(cullFace, BakedModelHelper.swapSprites(quads, spriteSwapper));
        }
        random.setSeed(42L);
        List quads = template.getQuads(null, null, random, ModelData.EMPTY, RenderType.solid());
    ...
    public static List<BakedQuad> swapSprites(List<BakedQuad> quads, UnaryOperator<TextureAtlasSprite> spriteSwapper) {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            for (int vertex = 0; vertex < 4; ++vertex) {
                float u = BakedQuadHelper.getU(vertexData, vertex);
                float v = BakedQuadHelper.getV(vertexData, vertex);
                BakedQuadHelper.setU(vertexData, vertex, newSprite.getU(SpriteShiftEntry.getUnInterpolatedU((TextureAtlasSprite)sprite, (float)u)));
                BakedQuadHelper.setV(vertexData, vertex, newSprite.getV(SpriteShiftEntry.getUnInterpolatedV((TextureAtlasSprite)sprite, (float)v)));
            }
