...
public class PartialItemModelRenderer {
    ...
    public void render(BakedModel model, int light) {
        this.render(model, Sheets.translucentCullBlockSheet(), light);
    }

    public void renderSolid(BakedModel model, int light) {
        this.render(model, Sheets.solidBlockSheet(), light);
    }

    public void renderGlowing(BakedModel model, int light) {
        this.render(model, RenderTypes.itemGlowingTranslucent(), light);
    }

    public void renderSolidGlowing(BakedModel model, int light) {
        this.render(model, RenderTypes.itemGlowingSolid(), light);
    }
    ...
    public void render(BakedModel model, RenderType type, int light) {
        ...
        if (!model.isCustomRenderer()) {
            VertexConsumer vc = ItemRenderer.getFoilBufferDirect((MultiBufferSource)this.buffer, (RenderType)type, (boolean)true, (boolean)this.stack.hasFoil());
            for (BakedModel pass : model.getRenderPasses(this.stack, false)) {
                this.renderBakedItemModel(pass, light, this.ms, vc);
            }
        } else {
            IClientItemExtensions.of((ItemStack)this.stack).getCustomRenderer().renderByItem(this.stack, this.transformType, this.ms, this.buffer, light, this.overlay);
        }
        this.ms.popPose();
    }
