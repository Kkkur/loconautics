...
extends AbstractContainerMenu {
    ...
    protected abstract T createOnClient(RegistryFriendlyByteBuf var1);
    ...
    protected void addPlayerSlots(int x, int y) {
        for (int row = 0; row < 3; ++row) {
            for (int col = 0; col < 9; ++col) {
                this.addSlot(this.createPlayerSlot(this.playerInventory, col + row * 9 + 9, x + col * 18, y + row * 18));
            }
        }
        for (int hotbarSlot = 0; hotbarSlot < 9; ++hotbarSlot) {
            this.addSlot(this.createPlayerSlot(this.playerInventory, hotbarSlot, x + hotbarSlot * 18, y + 58));
        }
    }

    protected Slot createPlayerSlot(Inventory inventory, int index, int x, int y) {
        return new Slot((Container)inventory, index, x, y);
    }
