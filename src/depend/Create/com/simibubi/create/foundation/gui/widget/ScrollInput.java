...
extends AbstractSimiWidget {
    ...
    protected Component title = CreateLang.translateDirect("gui.scrollInput.defaultTitle", new Object[0]);
    protected final Component scrollToModify = CreateLang.translateDirect("gui.scrollInput.scrollToModify", new Object[0]);
    protected final Component shiftScrollsFaster = CreateLang.translateDirect("gui.scrollInput.shiftScrollsFaster", new Object[0]);
    ...
    public ScrollInput(int xIn, int yIn, int widthIn, int heightIn) {
        ...
        this.formatter = i -> Component.literal((String)String.valueOf(i));
    }
    ...
    public boolean mouseScrolled(double pMouseX, double pMouseY, double pScrollX, double pScrollY) {
        ...
        if (priorState != this.state) {
            if (!this.soundPlayed) {
                Minecraft.getInstance().getSoundManager().play((SoundInstance)SimpleSoundInstance.forUI((SoundEvent)AllSoundEvents.SCROLL_VALUE.getMainEvent(), (float)(1.5f + 0.1f * (float)(this.state - this.min) / (float)(this.max - this.min))));
            }
    ...
    protected void updateTooltip() {
        ...
        this.toolTip.add(this.title.plainCopy().withStyle(s -> s.withColor(HEADER_RGB.getRGB())));
        if (this.hint != null) {
            this.toolTip.add(this.hint.plainCopy().withStyle(s -> s.withColor(HINT_RGB.getRGB())));
        }
