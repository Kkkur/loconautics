...
implements Shapes.DoubleLineConsumer {
    private final CollisionList collisionList;
    public int offsetX = 0;
    public int offsetY = 0;
    public int offsetZ = 0;

    public CollisionList.Populate(CollisionList collisionList) {
        this.collisionList = collisionList;
    }

    public void consume(double x1, double y1, double z1, double x2, double y2, double z2) {
        this.append((double)this.offsetX + 0.5 * (x2 + x1), (double)this.offsetY + 0.5 * (y2 + y1), (double)this.offsetZ + 0.5 * (z2 + z1), 0.5 * (x2 - x1), 0.5 * (y2 - y1), 0.5 * (z2 - z1));
    }
    ...
    public void appendFrom(CollisionList collisionList, int bbIdx) {
        this.append(collisionList.centerX[bbIdx], collisionList.centerY[bbIdx], collisionList.centerZ[bbIdx], collisionList.extentsX[bbIdx], collisionList.extentsY[bbIdx], collisionList.extentsZ[bbIdx]);
    }
}
