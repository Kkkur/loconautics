...
public class ContinuousOBBCollider {
    ...
    public static CollisionResponse collideMany(CollisionList collidableBBs, CollisionList denseViableColliders, OrientedBB obb, Vec3 motion, float entityMaxStep, boolean doHorizontalPass) {
        ...
        for (int bbIdx = 0; bbIdx < collidableBBs.size; ++bbIdx) {
            if (Math.abs(obbCenter.x + motion.x - collidableBBs.centerX[bbIdx]) > collidableBBs.extentsX[bbIdx] + aabbInLocalX || Math.abs(obbCenter.y + motion.y - collidableBBs.centerY[bbIdx]) > collidableBBs.extentsY[bbIdx] + aabbInLocalY || Math.abs(obbCenter.z + motion.z - collidableBBs.centerZ[bbIdx]) > collidableBBs.extentsZ[bbIdx] + aabbInLocalZ) continue;
        ...
        for (boolean horizontalPass : Iterate.trueAndFalse) {
            ...
            for (int bbIdx = 0; bbIdx < denseViableColliders.size; ++bbIdx) {
                ...
                if (mf.separate(uA0, deltaX, extentsX, a00 * obbExtents.x + a01 * obbExtents.y + a02 * obbExtents.z, motion.x, true) || mf.separate(uA1, deltaY, extentsY, a10 * obbExtents.x + a11 * obbExtents.y + a12 * obbExtents.z, motion.y, true) || mf.separate(uA2, deltaZ, extentsZ, a20 * obbExtents.x + a21 * obbExtents.y + a22 * obbExtents.z, motion.z, true)) continue;
                Vec3 deltaEntityFrame = rotation.transformTransposed(deltaX, deltaY, deltaZ);
                if (mf.separate(uB0, deltaEntityFrame.x, extentsX * a00 + extentsY * a10 + extentsZ * a20, obbExtents.x, motion2.x, false) || mf.separate(uB1, deltaEntityFrame.y, extentsX * a01 + extentsY * a11 + extentsZ * a21, obbExtents.y, motion2.y, false) || mf.separate(uB2, deltaEntityFrame.z, extentsX * a02 + extentsY * a12 + extentsZ * a22, obbExtents.z, motion2.z, false)) continue;
