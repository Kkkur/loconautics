...
public class SimulatedRegistrate
extends CreateRegistrate {
    ...
    public static final Map<ResourceLocation, ResourceLocation> ITEM_TO_SECTION = new ConcurrentHashMap<ResourceLocation, ResourceLocation>();
    private static final Map<ResourceLocation, Supplier<ItemLike>> NAVIGATION_TARGET_ITEMS = new ConcurrentHashMap<ResourceLocation, Supplier<ItemLike>>();
    ...
    public <T> Codec<T> byNameCodecExpanded(ResourceKey<? extends Registry<T>> key) {
        return ResourceLocation.CODEC.flatXmap(resourceLoc -> {
            Object gatheredEntry = null;
            for (RegistryEntry entry : this.getAll(key)) {
                if (!entry.getId().equals(resourceLoc)) continue;
            ...
            return DataResult.error(() -> "Unknown registry element in " + String.valueOf(key) + ":" + String.valueOf(resourceLoc));
        }, T -> {
            ...
            return DataResult.error(() -> "Unknown registry element in " + String.valueOf(key) + ":" + String.valueOf(T));
        });
    }

    public static ResourceLocation sectionOf(Item item) {
        return ITEM_TO_SECTION.get(BuiltInRegistries.ITEM.getKey((Object)item));
    }
    ...
    protected <R, T extends R> RegistryEntry<R, T> accept(String name, ResourceKey<? extends Registry<R>> type, Builder<R, T, ?, ?> builder, NonNullSupplier<? extends T> creator, NonNullFunction<DeferredHolder<R, T>, ? extends RegistryEntry<R, T>> entryFactory) {
        ...
        if (type.equals((Object)Registries.ITEM)) {
            RegistryEntry itemEntry = entry;
            TAB_ITEMS.add(() -> ((RegistryEntry)itemEntry).get());
    ...
    public <T extends NavigationTarget> RegistryEntry<NavigationTarget, T> navTarget(String name, NonNullSupplier<T> navTableItem, Supplier<ItemLike> itemSupplier) {
        RegistryEntry entry = this.simple((CreateRegistrate)this.self(), name, SimRegistries.Keys.NAVIGATION_TARGET, navTableItem);
    ...
    public <T extends NavigationTarget> RegistryEntry<NavigationTarget, T> navTarget(String name, NonNullSupplier<T> navTableItem, ItemLike item) {
        return this.navTarget(name, navTableItem, () -> item);
    }

    public <T extends BlockPropertiesTooltip.Entry> RegistryEntry<BlockPropertiesTooltip.Entry, T> propertyTooltip(String name, NonNullSupplier<T> tooltipFunction) {
    ...
    public static void onAddDefaultComponents(BiConsumer<ItemLike, Consumer<DataComponentPatch.Builder>> modify) {
        for (Map.Entry<ResourceLocation, Supplier<ItemLike>> entry : NAVIGATION_TARGET_ITEMS.entrySet()) {
            ...
            modify.accept(item, builder -> builder.set(SimDataComponents.TARGET, (Object)target));
        }
    }
}
