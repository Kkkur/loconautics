...
public class ModifyRopeInstruction
extends TickingInstruction {
    ...
    public ModifyRopeInstruction(int ticks, RopeStrandElement rope, Vec3 from, Vec3 to, double length, double sog, double floorDistance) {
        super(false, ticks);
        this.rope = rope;
        this.targetPose = new PonderRopePose(JOMLConversion.toJOML((Position)from), JOMLConversion.toJOML((Position)to), length, sog, floorDistance);
    }

    public ModifyRopeInstruction(int duration, RopeStrandElement rope) {
        this(duration, rope, JOMLConversion.toMojang((Vector3dc)rope.scenePose.start), JOMLConversion.toMojang((Vector3dc)rope.scenePose.end), rope.scenePose.length, rope.scenePose.sog, rope.scenePose.floorHeight);
    }

    public ModifyRopeInstruction setStart(Vec3 start) {
        this.targetPose.start.set((Vector3dc)JOMLConversion.toJOML((Position)start));
        this.rope.scenePose.start.set((Vector3dc)JOMLConversion.toJOML((Position)start));
        return this;
    }

    public ModifyRopeInstruction setEnd(Vec3 end) {
        this.targetPose.end.set((Vector3dc)JOMLConversion.toJOML((Position)end));
        this.rope.scenePose.end.set((Vector3dc)JOMLConversion.toJOML((Position)end));
        return this;
    }
    ...
    public ModifyRopeInstruction start(CreateSceneBuilder scene) {
        scene.addInstruction((PonderInstruction)this);
        return this;
    }
