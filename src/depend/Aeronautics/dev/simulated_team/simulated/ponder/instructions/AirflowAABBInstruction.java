...
public class AirflowAABBInstruction
extends TickingInstruction {
    ...
    public void tick(PonderScene scene) {
        ...
        double length = Vec3.atLowerCornerOf((Vec3i)this.direction.getNormal()).dot(new Vec3(this.bb.getXsize(), this.bb.getYsize(), this.bb.getZsize()));
        length = Math.abs(length);
        AABB commonBB = this.bb.contract((double)this.direction.getStepX() * length, (double)this.direction.getStepY() * length, (double)this.direction.getStepZ() * length);
        ...
        for (int i = startIndex; i <= endIndex; ++i) {
            double position = offset - (float)i * this.spacing;
            AABB currentBB = commonBB.move(Vec3.atLowerCornerOf((Vec3i)this.direction.getNormal()).scale(position));
            scene.getOutliner().chaseAABB((Object)(this.hash + i), currentBB).lineWidth(0.03125f).colored(this.color.getColor());
        }
    }
}
