...
public class CustomAnimateElementInstruction<T extends PonderSceneElement>
extends TickingInstruction {
    ...
    protected CustomAnimateElementInstruction(ElementLink<T> link, Vec3 totalDelta, int ticks, BiConsumer<T, Vec3> setter, Function<T, Vec3> getter, UnaryOperator<Float> positionFunc) {
        super(false, ticks);
        this.link = link;
        this.setter = setter;
        this.getter = getter;
    ...
    public void tick(PonderScene scene) {
        ...
        float P1 = ((Float)this.positionFunc.apply(Float.valueOf((float)time / (float)this.totalTicks))).floatValue();
        float P2 = ((Float)this.positionFunc.apply(Float.valueOf((float)(time + 1) / (float)this.totalTicks))).floatValue();
        float delta = P2 - P1;
