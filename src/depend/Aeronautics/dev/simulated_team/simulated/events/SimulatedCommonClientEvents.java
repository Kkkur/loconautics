...
public class SimulatedCommonClientEvents {
    ...
    public static InteractCallback.Result onBeforeMouseInput(InteractCallback.Input input, int modifiers, int action) {
        ...
        for (InteractCallback interactCallback : SimClickInteractions.CLICK_INTERACTION_ENTRIES) {
            ...
            if (InteractCallback.Result.empty().equals(returnEvent)) continue;
            return returnEvent;
        }
        return InteractCallback.Result.empty();
    }
    ...
    public static void onRenderLevelStage(VeilRenderLevelStageEvent.Stage stage, LevelRenderer levelRenderer, MultiBufferSource.BufferSource bufferSource, MatrixStack matrixStack, Matrix4fc matrix4fc, Matrix4fc matrix4fc1, int i, DeltaTracker deltaTracker, Camera camera, Frustum frustum) {
    ...
    public static InteractCallback.Result onMouseScroll(double deltaX, double deltaY) {
        if (Minecraft.getInstance().screen == null) {
            for (InteractCallback interactCallback : SimClickInteractions.CLICK_INTERACTION_ENTRIES) {
                ...
                if (InteractCallback.Result.empty().equals(result)) continue;
                return result;
            }
        }
        return InteractCallback.Result.empty();
    ...
    public static void preClientTick(Minecraft instance) {
        ...
        if (delta != 0.0) {
            SimulatedCommonClientEvents.onMouseScroll(0.0, delta);
        }
    }
    ...
    public static void appendTooltip(ItemStack stack, TooltipFlag iTooltipFlag, @Nullable Player player, List<Component> itemTooltip) {
        BlockPropertiesTooltip.Condition propertiesCondition = (BlockPropertiesTooltip.Condition)((Object)SimConfigService.INSTANCE.client().itemConfig.displayProperties.get());
