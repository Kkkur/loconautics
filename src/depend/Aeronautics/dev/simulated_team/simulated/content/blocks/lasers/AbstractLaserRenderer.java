...
public abstract class AbstractLaserRenderer<T extends AbstractLaserBlockEntity>
extends SmartBlockEntityRenderer<T> {
    public AbstractLaserRenderer(BlockEntityRendererProvider.Context context) {
        super(context);
    }
    ...
    protected void renderSafe(T blockEntity, float partialTicks, PoseStack pose, MultiBufferSource buffer, int light, int overlay) {
        ...
        LaserBehaviour laser = blockEntity.getAllBehaviours().stream().filter(behaviour -> behaviour instanceof LaserBehaviour).map(behaviour -> (LaserBehaviour)((Object)behaviour)).findFirst().orElse(null);
    ...
    public float getLaserLength(LaserBehaviour laser) {
        ...
        if (hr != null && !hr.getType().equals((Object)HitResult.Type.MISS)) {
            Vec3 hitPos = hr.getLocation();
            if (laser.getVirtualHitPos() != Vec3.ZERO) {
                hitPos = laser.getVirtualHitPos();
            }
            laserRange = (float)Math.sqrt(Sable.HELPER.distanceSquaredWithSubLevels(SableDistUtil.getClientLevel(), (Position)positions.getFirst(), (Position)hitPos)) - 0.1f;
    ...
    protected void transformPose(T blockEntity, LaserBehaviour laser, PoseStack pose) {
        Direction facing = ((AbstractLaserBlockEntity)((Object)blockEntity)).getDirection();
        pose.translate(0.5, 0.5, 0.5);
        ((PoseTransformStack)TransformStack.of((PoseStack)pose).rotate((Quaternionfc)facing.getRotation()).rotateXDegrees(-90.0f)).translate(0.0, 0.0, 0.4375);
    ...
    protected void createLaser(Vector4f color, PoseStack pose, MultiBufferSource buffer, float maxLength, float length) {
        ...
        if (buffer instanceof SuperRenderTypeBuffer) {
            SuperRenderTypeBuffer superRenderTypeBuffer = (SuperRenderTypeBuffer)buffer;
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            builder.addVertex(matrix, 0.0f, 0.0f, 0.0f).setColor(red, green, blue, alpha).setUv(0.0f, endU).setLight(0xF000F0).setOverlay(OverlayTexture.NO_OVERLAY).setNormal(0.0f, 1.0f, 0.0f);
            builder.addVertex(matrix, 1.0f, 0.0f, 0.0f).setColor(red, green, blue, alpha).setUv(0.0f, endU).setLight(0xF000F0).setOverlay(OverlayTexture.NO_OVERLAY).setNormal(0.0f, 1.0f, 0.0f);
            builder.addVertex(matrix, 1.0f + offset, -offset, length + 0.5f).setColor(red, green, blue, endAlpha).setUv(endU, endU).setLight(0xF000F0).setOverlay(OverlayTexture.NO_OVERLAY).setNormal(0.0f, 1.0f, 0.0f);
            builder.addVertex(matrix, -offset, -offset, length + 0.5f).setColor(red, green, blue, endAlpha).setUv(endU, endU).setLight(0xF000F0).setOverlay(OverlayTexture.NO_OVERLAY).setNormal(0.0f, 1.0f, 0.0f);
            pose.translate(0.5, 0.5, 0.5);
