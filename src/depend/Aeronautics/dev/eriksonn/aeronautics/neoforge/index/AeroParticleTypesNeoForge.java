...
public class AeroParticleTypesNeoForge {
    public static final DeferredRegister<ParticleType<?>> PARTICLE_TYPES = DeferredRegister.create((Registry)BuiltInRegistries.PARTICLE_TYPE, (String)"aeronautics");

    public static void registerEventListeners(IEventBus modEventBus) {
        for (AeroParticleTypes type : AeroParticleTypes.values()) {
            String name = CreateLang.asId((String)type.name());
