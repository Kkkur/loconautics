...
public class MergedMassTracker
implements MassData {
    ...
    public void update(float partialPhysicsTick) {
        ...
        ObjectCollection<KinematicContraption> contraptions = this.subLevel.getPlot().getContraptions();
        ...
        SableMathUtils.fmaInertiaTensor((Vector3dc)localShift, this.selfTracker.getMass(), this.inertiaTensor);
        for (KinematicContraption contraption : contraptions) {
            MassTracker contraptionMassData = contraption.sable$getMassTracker();
            Vector3d localPos = contraption.sable$getPosition(partialPhysicsTick).sub((Vector3dc)this.centerOfMass, new Vector3d());
            SableMathUtils.fmaInertiaTensor((Vector3dc)localPos, contraptionMassData.getMass(), this.inertiaTensor);
            Quaterniond contraptionOrientation = contraption.sable$getOrientation(partialPhysicsTick);
            Matrix3d localInertiaTensor = new Matrix3d().rotateLocal((Quaterniondc)contraptionOrientation.conjugate(new Quaterniond())).mulLocal(contraptionMassData.getInertiaTensor()).rotateLocal((Quaterniondc)contraptionOrientation);
            this.inertiaTensor.add((Matrix3dc)localInertiaTensor);
        }
    ...
    private void uploadData() {
        if (!(this.centerOfMass == null || this.mass == this.lastMass && Objects.equals(this.lastCenterOfMass, this.centerOfMass) && Objects.equals(this.lastInertiaTensor, this.inertiaTensor))) {
            if (this.lastCenterOfMass == null || this.lastInertiaTensor == null) {
                this.lastCenterOfMass = new Vector3d((Vector3dc)this.centerOfMass);
                this.lastInertiaTensor = new Matrix3d((Matrix3dc)this.inertiaTensor);
            }
            ServerLevel level = this.subLevel.getLevel();
            ServerSubLevelContainer container = SubLevelContainer.getContainer(level);
            SubLevelPhysicsSystem physicsSystem = container.physicsSystem();
            Vector3d movement = this.centerOfMass.sub((Vector3dc)this.lastCenterOfMass, new Vector3d());
            physicsSystem.updatePose(this.subLevel);
            Pose3d pose = this.subLevel.logicalPose();
            physicsSystem.getPipeline().teleport(this.subLevel, (Vector3dc)pose.position().add((Vector3dc)pose.orientation().transform(movement)), (Quaterniondc)pose.orientation());
            pose.rotationPoint().set((Vector3dc)this.centerOfMass);
    ...
    private void setPreviousValues() {
        if (this.centerOfMass == null) {
            ...
            this.lastCenterOfMass.set((Vector3dc)this.centerOfMass);
            this.lastInertiaTensor.set((Matrix3dc)this.inertiaTensor);
        }
