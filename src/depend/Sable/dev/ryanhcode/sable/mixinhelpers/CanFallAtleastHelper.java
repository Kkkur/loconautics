...
public class CanFallAtleastHelper {
    public static Vector3d canFallAtleastWithSubLevels(Level level, AABB aabb) {
        ...
        Iterable<SubLevel> intersecting = Sable.HELPER.getAllIntersecting(level, (BoundingBox3dc)considerationBounds);
        LevelReusedVectors sink = ((LevelExtension)level).sable$getJOMLSink();
        sink.entityBoxOrientation.identity();
        OrientedBoundingBox3d entityBoundsOBB = new OrientedBoundingBox3d((aabb.minX + aabb.maxX) / 2.0, (aabb.minY + aabb.maxY) / 2.0, (aabb.minZ + aabb.maxZ) / 2.0, aabb.getXsize() - 0.1, aabb.getYsize(), aabb.getZsize() - 0.1, (Quaterniondc)sink.entityBoxOrientation, sink);
        ...
        for (SubLevel subLevel : intersecting) {
            ...
            localBounds.minY -= 1.0;
            Iterable blocks = BlockPos.betweenClosed((BlockPos)BlockPos.containing((double)localBounds.minX, (double)(localBounds.minY - 1.0), (double)localBounds.minZ), (BlockPos)BlockPos.containing((double)localBounds.maxX, (double)localBounds.maxY, (double)localBounds.maxZ));
            ...
            for (BlockPos block : blocks) {
                ...
                VoxelShape voxelShape = state.getCollisionShape((BlockGetter)level, block);
                ...
                while (iterator.hasNext()) {
                    ...
                    if (!(satResult.lengthSquared() > 0.0) || satResult.x() == Double.MAX_VALUE || satResult.y() == Double.MAX_VALUE || satResult.z() == Double.MAX_VALUE) continue;
                    return satResult;
                }
            }
        }
