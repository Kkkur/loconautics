 ...
 *  foundry.veil.api.client.editor.Inspector
    ...
    public static void init() {
        VeilEventPlatform.INSTANCE.onVeilRendererAvailable(renderer -> {
            if (VeilRenderSystem.hasImGui()) {
                ...
                editorManager.add((Inspector)new SubLevelContainerInspector());
            }
        });
        VeilEventPlatform.INSTANCE.onVeilAddShaderProcessors((provider, registry) -> {
            registry.addPreprocessor((ShaderPreProcessor)new SableDynamicDirectionalShadingPreProcessor(), false);
            registry.addPreprocessor((ShaderPreProcessor)new SableDynamicSkyLightShadowPreProcessor(), false);
            registry.addPreprocessor((ShaderPreProcessor)new SableWaterOcclusionPreProcessor(), false);
            registry.addPreprocessor((ShaderPreProcessor)new FancySubLevelShaderProcessor(), false);
        });
