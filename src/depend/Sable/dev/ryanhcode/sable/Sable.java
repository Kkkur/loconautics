...
public final class Sable {
    ...
    public static void init() {
        ...
        LOGGER.info("{} loaded!", (Object)MOD_NAME);
    }

    public static PhysicsPipeline createPhysicsPipeline(ServerLevel level) {
    ...
    private static String getWittierComment() {
        try {
            if (LocalDate.now().getDayOfWeek() == DayOfWeek.SUNDAY && Util.getMillis() % 2L == 0L) {
                return "It's sable sunday";
            }
            return WITTIER_COMMENTS.get((int)(Util.getMillis() % (long)WITTIER_COMMENTS.size()));
        }
        catch (Throwable t) {
