...
public class SimpleCulledRenderRegionBuilder {
    ...
    public SimpleCulledRenderRegionBuilder(int gridSize) {
        ...
        this.cubes = new LinkedList<Cube>();
    ...
    private boolean hasVoxel(int x, int y, int z) {
        for (int i = 0; i < 1; ++i) {
            ...
            if (!this.grid.get(this.getGridIndex(vx, vy, vz))) continue;
            return true;
        }
        return false;
    }

    protected boolean shouldFaceRender(@NotNull Cube cube, @NotNull Direction direction) {
        int x0 = cube.x;
        int y0 = cube.y;
        int z0 = cube.z;
        ...
        return switch (direction) {
            ...
            case Direction.DOWN -> {
                for (int x = x0; x < x1; ++x) {
                    for (int z = z0; z < z1; ++z) {
                        if (this.hasVoxel(x, y0 - 1, z)) continue;
                        yield true;
                    }
                }
                yield false;
            ...
            case Direction.UP -> {
                for (int x = x0; x < x1; ++x) {
                    for (int z = z0; z < z1; ++z) {
                        if (this.hasVoxel(x, y1, z)) continue;
                        yield true;
                    }
                }
                yield false;
            ...
            case Direction.NORTH -> {
                for (int x = x0; x < x1; ++x) {
                    for (int y = y0; y < y1; ++y) {
                        if (this.hasVoxel(x, y, z0 - 1)) continue;
                        yield true;
                    }
                }
                yield false;
            ...
            case Direction.SOUTH -> {
                for (int x = x0; x < x1; ++x) {
                    for (int y = y0; y < y1; ++y) {
                        if (this.hasVoxel(x, y, z1)) continue;
                        yield true;
                    }
                }
                yield false;
            ...
            case Direction.WEST -> {
                for (int z = z0; z < z1; ++z) {
                    for (int y = y0; y < y1; ++y) {
                        if (this.hasVoxel(x0 - 1, y, z)) continue;
                        yield true;
                    }
                }
                yield false;
            ...
            case Direction.EAST -> {
                for (int z = z0; z < z1; ++z) {
                    for (int y = y0; y < y1; ++y) {
                        if (this.hasVoxel(x1, y, z)) continue;
                        yield true;
                    }
                }
                yield false;
    ...
    private void mergeZ() {
        ...
        for (int i = 0; i < this.cubes.size(); ++i) {
            ...
            if ((length = i - startIndex - 1) > 0) {
                Cube start = this.cubes.get(startIndex);
                Cube end = this.cubes.get(startIndex + length);
                for (int j = 0; j <= length; ++j) {
                    this.cubes.remove(startIndex);
                }
                this.cubes.add(startIndex, new Cube(start.x, start.y, start.z, sizeX, start.sizeY, end.z - start.z + end.sizeZ));
            }
            startIndex = -1;
            i -= length + 1;
        }
    ...
    private void mergeY() {
        ...
        for (int i = 0; i < this.cubes.size(); ++i) {
            ...
            if ((length = i - startIndex - 1) > 0) {
                ...
                this.cubes.add(startIndex, new Cube(start.x, start.y, start.z, sizeX, end.y - start.y + end.sizeY, sizeZ));
            }
            startIndex = -1;
            i -= length + 1;
        }
    ...
    public void buildNoGreedy() {
        ...
        for (int y = 0; y < this.gridSize; ++y) {
            for (int z = 0; z < this.gridSize; ++z) {
                for (int x = 0; x < this.gridSize; ++x) {
                    if (!this.grid.get(this.getGridIndex(x, y, z))) continue;
