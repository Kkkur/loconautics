...
public class ForceTotal {
    ...
    public void applyForces(RigidBodyHandle handle) {
        boolean forceChanged = this.localForce.distanceSquared((Vector3dc)this.lastLocalForce) > 1.0E-5;
        boolean torqueChanged = this.localTorque.distanceSquared((Vector3dc)this.lastLocalTorque) > 1.0E-5;
        boolean wakeUp = forceChanged || torqueChanged;
        handle.applyLinearAndAngularImpulse((Vector3dc)this.localForce, (Vector3dc)this.localTorque, wakeUp);
        this.lastLocalForce.set((Vector3dc)this.localForce);
        this.lastLocalTorque.set((Vector3dc)this.localTorque);
    ...
    public void applyForceTotal(ForceTotal other) {
        this.localForce.add((Vector3dc)other.localForce);
        this.localTorque.add((Vector3dc)other.localTorque);
    }
    ...
    public void applyImpulseAtPoint(MassData massTracker, Vector3dc position, Vector3dc force) {
        ...
        this.localTorque.add((Vector3dc)this.temp.cross(force));
    }
    ...
    public void applyImpulseAtPoint(MassTracker massTracker, Vec3 position, Vec3 force) {
        this.applyImpulseAtPoint((MassData)massTracker, (Vector3dc)JOMLConversion.toJOML((Position)position), (Vector3dc)JOMLConversion.toJOML((Position)force));
    }
}
