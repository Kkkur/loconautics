...
public class VanillaSingleSubLevelRenderData
implements SubLevelRenderData {
    ...
    public void renderSingleBlock(RenderType layer, VertexConsumer consumer, Matrix4f modelView, double camX, double camY, double camZ) {
        ...
        List<RenderType> renderLayers = SableSubLevelRenderPlatform.INSTANCE.getRenderLayers(LEVEL_WRAPPER, bakedModel, this.singleBlockState, this.singleBlockPos, RANDOM);
        ...
        Vector3d renderCOR = renderRot.transform(CENTER_OF_ROT.set(renderPose.rotationPoint()).sub((double)this.singleBlockPos.getX(), (double)this.singleBlockPos.getY(), (double)this.singleBlockPos.getZ()));
        ...
        stack.last().pose().mul((Matrix4fc)modelView).mul((Matrix4fc)transform);
    ...
    public void rebuild() {
        ...
        this.singleBlockState = blockState = this.subLevel.getLevel().getBlockState(pos);
