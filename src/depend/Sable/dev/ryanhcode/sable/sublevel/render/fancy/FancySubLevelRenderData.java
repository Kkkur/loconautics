...
public class FancySubLevelRenderData
implements SubLevelRenderData {
    ...
    private final List<FancySubLevelSectionCompiler.RenderSection> dirtyRenderSections = new LinkedList<FancySubLevelSectionCompiler.RenderSection>();
            ...
            for (int z = minChunkPos.z(); z <= maxChunkPos.z(); ++z) {
                for (int x = minChunkPos.x(); x <= maxChunkPos.x(); ++x) {
                    for (int y = minChunkPos.y(); y <= maxChunkPos.y(); ++y) {
                        ...
                        this.renderSections[this.getIndex((int)x, (int)y, (int)z)] = section = new FancySubLevelSectionCompiler.RenderSection(SectionPos.of((int)x, (int)y, (int)z), (Vector3ic)minChunkPos);
    ...
    public void compileSections(PrioritizeChunkUpdates chunkUpdates, RenderRegionCache renderRegionCache, Camera camera) {
        ...
        for (FancySubLevelSectionCompiler.RenderSection section : this.dirtyRenderSections) {
            ...
            this.compiler.getScheduler().scheduleCompile(section, renderRegionCache.createRegion((Level)level, section.getPos()), distanceSq, this.occlusionData::addSection);
            section.setNotDirty();
        }
        this.dirtyRenderSections.clear();
    }
