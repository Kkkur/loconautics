...
public class SubLevelSerializer {
    ...
    private static CompoundTag serialize(ServerSubLevel subLevel, List<UUID> dependencies) {
        CompoundTag userDataTag;
        CompoundTag tag = new CompoundTag();
        ServerLevelPlot plot = subLevel.getPlot();
        ListTag dependencyTags = new ListTag();
        for (UUID dependency : dependencies) {
            dependencyTags.add((Object)NbtUtils.createUUID((UUID)dependency));
        }
        Pose3d serializedPose = new Pose3d((Pose3dc)subLevel.logicalPose());
        Vector3dc selfCenterOfMass = subLevel.getSelfMassTracker().getCenterOfMass();
        serializedPose.position().set((Vector3dc)subLevel.logicalPose().transformPosition(new Vector3d(selfCenterOfMass)));
        serializedPose.rotationPoint().set(selfCenterOfMass);
        tag.putUUID("uuid", subLevel.getUniqueId());
        tag.put("plot", (Tag)plot.save());
        tag.put("pose", (Tag)SableNBTUtils.writePose3d(serializedPose));
        tag.put("world_bounds", (Tag)SableNBTUtils.writeBoundingBox(subLevel.boundingBox()));
        ...
        if (handle != null) {
            ...
            if (linearVelocity.lengthSquared() > 0.010000000000000002) {
                tag.put("linear_velocity", (Tag)SableNBTUtils.writeVector3d((Vector3dc)linearVelocity));
            }
            if (angularVelocity.lengthSquared() > Math.toRadians(1.0)) {
                tag.put("angular_velocity", (Tag)SableNBTUtils.writeVector3d((Vector3dc)angularVelocity));
            }
        }
        ...
        if (!dependencies.isEmpty()) {
            tag.put("loading_dependencies", (Tag)dependencyTags);
        }
        if ((userDataTag = subLevel.getUserDataTag()) != null) {
            tag.put("user_data", (Tag)userDataTag);
        }
        return tag;
    }
    ...
    public static SubLevelData fromData(CompoundTag tag) {
        ...
        ObjectArrayList dependencies = List.of();
        if (tag.contains("loading_dependencies")) {
            ...
            for (Tag dependencyUUIDTag : dependencyUUIDS) {
                UUID dependencyUUID = NbtUtils.loadUUID((Tag)dependencyUUIDTag);
                dependencies.add(dependencyUUID);
            }
        }
        return new SubLevelData(uuid, SableNBTUtils.readBoundingBox(tag.getCompound("world_bounds")), SableNBTUtils.readPose3d(tag.getCompound("pose")), (List<UUID>)dependencies, tag);
    }
    ...
    public static ServerSubLevel fullyLoad(ServerLevel level, SubLevelData halfLoadedSubLevel) {
        ...
        if (Double.isNaN(position.x) || Double.isNaN(position.y) || Double.isNaN(position.z) || Double.isNaN(cor.x) || Double.isNaN(cor.y) || Double.isNaN(cor.z)) {
            Sable.LOGGER.error("Failed to load sub-level, invalid pose: {}", (Object)pose);
            return null;
        }
        ...
        try {
            ...
            Sable.LOGGER.error("Failed to load sub-level, skipping", (Object)halfLoadedSubLevel, (Object)e);
            return null;
        }
        ServerLevelPlot plot = subLevel.getPlot();
        plot.load(plotTag);
        if (plot.getBoundingBox() == BoundingBox3i.EMPTY || plot.getBoundingBox().volume() <= 0) {
            Sable.LOGGER.error("Failed to load sub-level, invalid plot bounds: {}", (Object)(plot.getBoundingBox() == BoundingBox3i.EMPTY ? "EMPTY" : plot.getBoundingBox()));
        ...
        subLevel.logicalPose().set((Pose3dc)pose);
        physicsSystem.getPipeline().teleport(subLevel, (Vector3dc)position, (Quaterniondc)pose.orientation());
    ...
    public static SubLevelData toData(ServerSubLevel subLevel, @NotNull List<UUID> dependencies) {
        ObjectArrayList filteredDependencies = new ObjectArrayList(dependencies);
        filteredDependencies.remove(subLevel.getUniqueId());
        CompoundTag tag = SubLevelSerializer.serialize(subLevel, (List<UUID>)filteredDependencies);
        return new SubLevelData(subLevel.getUniqueId(), new BoundingBox3d(subLevel.boundingBox()), new Pose3d((Pose3dc)subLevel.logicalPose()), (List<UUID>)filteredDependencies, tag);
    }
}
